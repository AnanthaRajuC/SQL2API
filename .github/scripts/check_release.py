...
It fails when
  * the tag is not `v<sql2api.__version__>` - i.e. the tag was pushed on a commit without the version bump,
...
def package_version(root):
    text = (root / 'sql2api' / '__init__.py').read_text()
    match = re.search(r'''^__version__\s*=\s*['"]([^'"]+)['"]''', text, re.M)
    if not match:
        raise SystemExit('Could not find __version__ in sql2api/__init__.py')
